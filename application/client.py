...
from exceptions import ExitTryExcept, NoTargetError, NoEncryptionKeyError
...
class Client(object):
                        ...
                        while True:
                            ...
                            if len(full_msg) - HEADERSIZE == msg_len:
                                ...
                                if self._utf8:
                                    ...
                                    self._latest_statuses.append(
                                        Status(8, (full_message_dec, self._target))
                                    )
                                raise ExitTryExcept
                    except ExitTryExcept:
                        pass
    ...
    def send_msg(self, message: str):
        ...
        split_size_enc = self.headerify(
            encryption.encrypt(bytes(str(split_size), "utf-8"), self._peer_public_key)
        )
        ...
        for i in range(split_size):
            if self._utf8:
                ...
                self._s.send(
                    self.headerify(
                        encryption.encrypt(
                            message[446 * i : 446 * (i + 1)], self._peer_public_key
                        )
                    )
                )
