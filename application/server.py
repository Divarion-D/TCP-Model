...
import encryption
from exceptions import ExitTryExcept, NoEncryptionKeyError, NoTargetError
...
class Server(object):
    ...
    def __init__(self, port: int = 2006):
        ...
        self._private_key = encryption.gen_private_key()
    ...
    def _connection(self):
        ...
        while True:
            ...
            cs.send(
                self.headerify(
                    encryption.get_public_key_text(
                        encryption.get_public_key(self._private_key)
                    )
                )
            )
                ...
                try:
                    ...
                    while True:
                        ...
                        if len(full_msg) - HEADERSIZE == msg_len:
                            ...
                            full_msg_len = int(
                                encryption.decrypt(
                                    full_msg[HEADERSIZE:], self._private_key
                                )
                            )
                            actual_full_message = []
                            ...
                            for i in actual_full_message:
                                full_message_dec += encryption.decrypt(
                                    i, self._private_key
                                )
                            if self._utf8:
                                self._latest_statuses.append(
                                    Status(8, (full_message_dec.decode("utf-8"), addr))
                                )
                            else:
                                self._latest_statuses.append(
                                    Status(8, (full_message_dec, addr))
                                )
                            raise ExitTryExcept
                except ExitTryExcept:
                    pass
    ...
    def send_msg(self, message: str, target: tuple):
        ...
        if target not in self._peer_public_keys:
            raise NoEncryptionKeyError(
                "There is no public key from the target specified to encrypt with."
            )
        ...
        split_size_enc = self.headerify(
            encryption.encrypt(
                bytes(str(split_size), "utf-8"), self._peer_public_keys[target]
            )
        )
        ...
        for i in range(split_size):
            if self._utf8:
                self._sockets[target].send(
                    self.headerify(
                        encryption.encrypt(
                            bytes(message[446 * i : 446 * (i + 1)], "utf-8"),
                            self._peer_public_keys[target],
                        )
                    )
                )
            else:
